...
async function selectCourse(Cookie) {
    for (const courseid of courseidList) {
        const res = await axios.post("https://www.cdjxjy.com/ashx/SelectApi.ashx?a=SelectCourse", "courseid=" + courseid, {
            headers: {
                Cookie: Cookie
            }
        })
...
async function createLogin() {
    return axios.post("https://www.cdjxjy.com/ashx/BaseApi.ashx?a=CreateLogin").then(res => {
        return res.data
    })
}

async function wxQrloginCheck(codeid) {
    return axios.post("https://www.cdjxjy.com/ashx/weixindispose.ashx?a=wxQrloginCheck", "codeid=" + codeid).then(res => {
        return res.data
    })
}

async function wxQrlogin(openid) {
    return axios.post("https://www.cdjxjy.com/Project/API/UserAPI.ashx?a=wxteacherlogin", "openid=" + openid).then(res => {
        ...
        selectCourse(userCookies).then(() => {
            ...
            const dataIndex = dataArr.findIndex((item) => item.userid === res.data.userid)
            if (dataIndex >= 0) {
...
// wxQrlogin("oH-jTv7vuBcuJ-mAhxc2m6qVI9sQ").then(data => {
//     console.log(data)
// })
